...
import { AddCategorySelect } from "@/components/ui/add-category-select";
...
export function AddExpenseDialog({
  ...
  const [costCode, setCostCode] = React.useState("material");
  ...
  function submit(e: React.FormEvent) {
    ...
    addTransaction({
      ...
      costCode,
      category,
      note: note.trim() || categoryLabel[category] || catStore.customExpenseCategories[category] || category,
    });
    setAmount("");
    setNote("");
    onClose();
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1.5">
            ...
            <AddCategorySelect
              ...
              options={[
                ...DEFAULT_CATEGORIES,
                ...Object.entries(catStore.customExpenseCategories).map(([k, l]) => ({ key: k, label: l })),
              ]}
              onAdd={catStore.addExpenseCategory}
            />
          </div>
          <div className="space-y-1.5">
            ...
            <AddCategorySelect
              ...
              options={[
                ...DEFAULT_COST_CODES,
                ...Object.entries(catStore.customCostCodes).map(([k, l]) => ({ key: k, label: l })),
              ]}
              onAdd={catStore.addCostCode}
            />
          </div>
        </div>
        <div className="space-y-1.5">
    ...
    <Dialog open={open} onClose={onClose} title="Add Attendance" description="Updates the labour attendance chart live.">
      <form onSubmit={submit} className="space-y-4">
        <div className="space-y-1.5">
          ...
          <Input id="a-date" type="date" value={date} max={today()} onChange={(e) => setDate(e.target.value)} />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1.5">
