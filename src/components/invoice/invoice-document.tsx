...
/** Pixel-faithful Keyvendors Tax Invoice (matches Quotation branding). */
export function InvoiceDocument({ s, c }: { s: InvoiceState; c: ComputedInvoice }) {
  return (
    <article
      id="invoice-doc"
      ...
      <div className="grid grid-cols-[1.5fr_1fr]">
        ...
        <div>
          <div className="flex h-24 flex-col justify-center px-3 text-[11px]">
            <p><span className="font-semibold">Invoice No:</span> &nbsp; {s.number || "—"}</p>
            <p><span className="font-semibold">DATE</span> &nbsp; {fmtDate(s.date)}</p>
            <p><span className="font-semibold">Due Date:</span> &nbsp; {fmtDate(s.dueDate)}</p>
          </div>
          ...
          {c.lines.map((l, i) => {
            ...
            <tr key={l.id} className="align-top">
              ...
              <td className="border border-slate-400 px-1 py-1 text-center tabular-nums">
                {lm === "qty" || lm === "qty_rate" ? "Lumpsum" : lm === "amount_only" ? "" : l.qty || ""}
              </td>
